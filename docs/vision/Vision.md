...
`modeller-agents` is built to be that place: the composition and orchestration root. It does not model transport networks, does not store durable knowledge, and does not implement a backend. It routes a governed request to the right method (agent, skill, or workflow), checks that method is authorized for the target repository, enforces a source-boundary and consent gate before anything executes, and requires artifact evidence before a workflow step can advance. This is not a proposal for a system that does not exist: routing, skills, gates, and a deterministic workflow runtime are built and exercised today. What remains a target is the complete, end-to-end proof that a request travels all the way from a governed context envelope through a registered backend to a validated result and back, with a real backend smoke passing.
...
The target experience for an operator or an orchestrating agent: a context envelope states intent, target, risk, and consent; `route` resolves it to a bundle, a skill, and the reference packs that authorize that skill; a deterministic workflow gate requires the artifacts a step needs before the next step opens; and a backend is only called through a registered, contract-validated seam, never through an ad hoc import of another repository's implementation.
...
| A subagent's "done" claim is, by design, not accepted as evidence | This is already the enforced target state: the deterministic workflow gate requires artifact-level evidence today, and this row records a proven discipline, not a gap |
...
The deterministic workflow (`workflow init` / `status` / `check` / `advance`) turns an objective into a sequence of gated steps, each owned by a named role (`orchestrator`, `planner`, `architect`, `executor`, `documenter`), each requiring specific artifacts before the next step opens.
...
A vision for a composition root earns confidence by closing its own currently-documented readiness gaps on real, already-registered work, not by adding new scope. The natural bounded starting point is the repo's own strict-readiness blocker list.
...
Success would show that the routing and gating discipline already built here can carry a real request end to end through a registered backend and back, with every strict-readiness blocker code (`reference-pack-not-active`, `backend-not-active`, `backend-contract-unpinned`, `vendor-not-synced`, `vendor-not-pinned`, `schema-sibling-fallback`) cleared by evidence, not by relaxing the gate. Candidate baseline indicators: which blockers are open today, how long each remediation takes once scoped, and whether the resulting smoke evidence is stable across repeated runs.

### A way to begin

> This document offers a vision and a suggested starting point, not a request for commitment.

If the direction resonates, a modest next step would be a short brief scoped to exactly the strict-readiness blocker list in `docs/readiness/STRICT_READINESS.md`: which vendor gets synced first, who confirms the `aimsun-psp` remote and pipeline id for the smoke, and what "clean" looks like when `doctor --strict --json` is re-run. Where the organisation takes it from there, and at what pace, is entirely its own to decide.

---

## 9. Conclusion

This document does not claim `modeller-agents` is a finished orchestration platform. Routing, skill and reference-pack authorization, source-boundary checks, consent gating, and the deterministic artifact-gated workflow are built and exercised today; the complete Testudo-to-backend service loop, pinned vendors, active reference packs, and a proven backend smoke remain a target this repo's own readiness tooling tracks honestly rather than assumes.

What this is not: a second authority over knowledge, memory, or backend implementation, or an autonomous system that executes without consent. What it is: a deterministic, evidence-gated seam that lets the ecosystem's existing repositories keep their own authority while a request still gets routed, gated, and proven, one closed blocker at a time.
